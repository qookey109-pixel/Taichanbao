...
for token in ["lifecycle-v3-2.css", "lifecycle-v3-2.js", "V3.2 Registry Lifecycle", "V3.1 Registry Scale 50"]:
    assert token in loader, token

for token in [
    "V3.2 REGISTRY LIFECYCLE", "registry-expiry.json", "catalog.public.json",
    "registry.manifest.json", "lifecycle-section", "lifecycleNext", "life90",
    "data-life-window", "lifecycle-pill", "decorateCatalogCards", "deriveReport"
]:
...
assert within_90 >= 1, "V3.2 test fixture should exercise an upcoming expiry"

print(
    f"OK: V3.2 lifecycle registry=50 expired=0; "
