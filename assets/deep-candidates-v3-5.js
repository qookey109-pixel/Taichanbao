(() => {
  ...
  async function boot(){try{const [payload,catalog]=await Promise.all([j("data/deep_case.candidates.json"),j("data/catalog.public.json")]);const records=new Map((catalog.records||[]).map(r=>[r.id,r]));inject(payload,records);updateVersion();}catch(e){console.warn("V3.8 deep candidate review unavailable",e);}}
  ...
  function inject(payload,records){if(document.querySelector("#deepCandidates"))return;const anchor=document.querySelector("#enrichment")||document.querySelector("#lifecycle")||document.querySelector("#database");if(!anchor)return;const items=payload.items||[];const section=document.createElement("section");section.id="deepCandidates";section.className="deep-candidate-section sans";section.innerHTML=`
    <div class="deep-candidate-head"><div><div class="deep-candidate-kicker">V3.8 · CANDIDATE PROMOTION REVIEW</div><h2>候選審核可以通過，<br>圖片權利仍然可以擋住發布。</h2><p>KD-884HP0 與 KD-703HP1 都已通過候選層 editorial promotion review。這只代表值得做深度專題；圖片重用授權與正式 publication editorial review 仍是獨立 Gate。</p></div><div class="deep-candidate-status"><b>CANDIDATE STATUS</b><strong>${items.length} 筆已登錄候選</strong><span>${items.filter(i=>i.candidate_status.startsWith("blocked")).length} 筆目前被資產 Gate 阻擋 · 正式發布 0</span></div></div>
    <div class="deep-candidate-grid">${items.map(i=>card(i,records.get(i.source_record_id)||{})).join("")}</div>
    <div class="deep-candidate-note">Candidate editorial review ≠ publication editorial review。圖片授權未完成時，即使候選審核 PASS，也必須維持 unpublished。</div>`;
  ...
  function gateLabel(k){return({brand_identity:"品牌身分",exact_model_identity:"精確型號",mit_manufacturing_evidence:"MIT 製造證據",current_sale_or_supply:"現售／供應",exact_official_product_page:"官方型號頁",key_conflict_review:"重大衝突",candidate_editorial_review:"候選編輯審核",image_rights:"圖片權利",publication_editorial_review:"發布編輯審核",formal_publication:"正式發布"})[k]||k;}
